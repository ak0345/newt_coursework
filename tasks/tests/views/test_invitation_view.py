...
class InviteUserViewTest(TestCase):
    def setUp(self):
        ...
        self.team = Team.objects.create(
            team_name="Test Team", description="A test team", team_owner=self.user1
        )
    ...
    def test_invite_user_view_with_existing_user(self):
        ...
        self.assertTrue(
            Invitation.objects.filter(
                user_requesting_to_join=self.user2, team_to_join=self.team
            ).exists()
        )
    ...
    def test_invite_user_view_with_nonexistent_user(self):
        ...
        response = self.client.post(
            self.invite_url, {"input_username": "nonexistentuser"}
        )
        self.assertEqual(response.status_code, 302)
        messages = list(get_messages(response.wsgi_request))
